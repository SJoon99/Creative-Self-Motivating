...
class OmniOpsExtension(omni.ext.IExt):
            ...
            with ui.ScrollingFrame(style={"background_color": PANEL_BG}):
                with ui.VStack(spacing=8, height=0):
                    ...
                    with ui.HStack(spacing=12, height=0):
                        with self._dashboard_card(width=1120):
                            ui.Label("Plan Explanations", height=22, style={"color": TEXT_MAIN, "font_size": 16})
                            ui.Label(
                                "Score is the Twin simulation fit after Gemma/RAG proposes each Blueprint; ship is days earlier/later than the fixed Baseline.",
                                height=20,
                                word_wrap=True,
                                style={"color": TEXT_MUTED},
                            )
                            with ui.HStack(spacing=8, height=160):
                                for _ in range(3):
    ...
    def _score_row(self):
        row = {}
        with ui.Frame(width=296, height=154, style={"background_color": CARD_BG_DARK, "border_radius": 8, "padding": 8}):
            with ui.VStack(spacing=3):
                ...
                row["tradeoff"] = ui.Label("-", height=36, word_wrap=True, style={"color": TEXT_MUTED})
        return row
    ...
    def _refresh_scores(self):
        ...
        for idx, widgets in enumerate(self._score_rows):
            ...
            row = rows[idx]
            blueprint_id = str(row.get("blueprintId") or row.get("id") or "")
            marker = "★" if blueprint_id == recommended_id else ("●" if blueprint_id == active_id else "")
