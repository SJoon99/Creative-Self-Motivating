...
if [[ -f "$COMMON_SH" ]]; then
  # Reuse the same TwinX Gemma/RAG defaults as the streaming/runtime scripts.
  # This keeps the OmniOps GUI Generate Blueprint button on the live internal
  # RAG LB unless the operator overrides SMARTFARM_RAG_* explicitly.
  # shellcheck source=scripts/smartfarm-twin-common.sh
  source "$COMMON_SH"
  APP_HOME="$SMARTFARM_APP_HOME"
fi
APP_FILE="$APP_HOME/source/apps/joon.smartfarm_omniops.kit"
...
stop_conflicts() {
  ...
  if [[ "$KEEP_EXISTING" == "1" ]]; then
    ...
    echo "WARNING: OmniOps API calls may hit the old process, so the GUI scene may stay empty." >&2
    return 0
  fi
  echo "Stopping existing SmartFarm Kit process(es) so OmniOps controls its own GUI stage: ${pids[*]}"
...
stop_conflicts
cd "$APP_HOME"
echo "Starting SmartFarm OmniOps GUI app. Existing strawberry farm scene root should be /World/SmartFarm."
